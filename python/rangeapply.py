...
def main():
    ...
    parser.add_option("--srcRoot", help="Source root directory to rename", action="store", dest="srcRoot")
    parser.add_option("--srcRangeMap", help="Source range map generated by srg2source", action="store", dest="srcRangeMap")
    parser.add_option("--srgFiles", help="Symbol map file(s), separated by ','", action="store", dest="srgFiles")
    parser.add_option("--git", help="Command to invoke git", action="store", dest="git", default="git")
    parser.add_option("--lvRangeMap", help="Original source range map generated by srg2source, for renaming local variables", action="store", dest="lvRangeMap")  # TODO: csv instead?
    parser.add_option("--mcpConfDir", help="MCP configuration directory, for renaming parameters", action="store", dest="mcpConfDir")
    parser.add_option("--excFiles", help="Parameter map file(s), separated by ','", action="store", dest="excFiles")
    parser.add_option("--no-rewriteFiles", help="Disable rewriting files with new symbol mappings", action="store_true", dest="rewriteFiles", default=True)
    parser.add_option("--no-renameFiles", help="Disable renaming files with new filenames", action="store_false", dest="renameFiles", default=True)
    parser.add_option("--no-dumpRenameMap", help="Disable dumping symbol rename map before renaming", action="store_false", dest="dumpRenameMap", default=True)
